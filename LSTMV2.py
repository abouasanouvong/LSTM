...
def main():
    ...
    #Time Based features extract calendar information from dates to help the model understand when
    # something happens, not just what happens. They encode cyclic patterns that occur regularly based on the calendar.
    ...
    # Lag Features are historical values of your vairables fomr previous time periods helps determine previous values
    for lag in [1, 3, 7, 14]:
        df_groceries[f'price_lag_{lag}'] = df_groceries['Price'].shift(lag)
        df_groceries[f'competitor_price_lag_{lag}'] = df_groceries['Competitor Pricing'].shift(lag)

    # Rolling Statistic calculate metrics (mean, standard, deviation, etc) over a moving window of data points.
    # They summarize behaviour over a period of time, giving smoothed trends and volatility measures.
    for window in [7, 14, 30]:
        ...
        df_groceries[f'competitor_price_ma_{window}'] = df_groceries['Competitor Pricing'].rolling(window=window, center=True).mean()
        df_groceries[f'competitor_price_std_{window}'] = df_groceries['Competitor Pricing'].rolling(window=window, center=True).mean()
    ...
    #Fill missing values using ffill forward to carry last value fwd to fill missing values and bfill to carry next valid
    # value backward
    ...
    feature_cols = [col for col in df_groceries.columns if col not in ['Date','Category', 'Store ID', 'Product ID', 'Region', 'Weather Condition', 'Seasonality', 'Epidemic', 'Units Ordered']]
    ...
    # Calculate split point for 70 Train/15 Test/ 15 Validation split
    ...
    for i in range(60, len(train_data)):
        X_train.append(np.delete(train_data[i - 60:i, :], target_col_idx, axis=1))  # All features except target
    ...
    for i in range(60, len(val_data)):
        X_val.append(np.delete(val_data[i - 60:i, :], target_col_idx, axis=1))  # All features except target
    ...
    for i in range(60, len(test_data)):
        X_test.append(np.delete(test_data[i - 60:i, :], target_col_idx, axis=1)) # All features except target
    ...
    # # # Second Layer distills the important patterns from the first layer and prepares to make a final prediction
    ...
    # Define callbacks for better training these get called automatically at specific points during training
    #aftr epochs, when validation loss stops improving, etc.
    callbacks = [
        EarlyStopping(
            monitor='val_loss',
            patience=15,
        ...
        #Early stopping, stops training when validation loss stops improving for 15 epochs
    ...
    # training
    history = model.fit(X_train, y_train, validation_data=(X_val, y_val), batch_size=32, epochs=50, callbacks=callbacks, verbose=1)
    ...
    df_test_aligned['Predictions'] = predictions
    df_test_aligned['Y_test']   = y_test

    # Create comprehensive plots
    fig, axes = plt.subplots(2, 2, figsize=(20, 12))

    # Plot 1: Predication vs Test data
    axes[0, 0].plot(df_test_aligned['Date'], df_test_aligned['Y_test'], label='Actual Test', color='green')
    axes[0, 0].plot(df_test_aligned['Date'], df_test_aligned['Predictions'], label='Predictions', color='red',
                    linestyle='--')
    axes[0, 0].set_title('Price Prediction vs Test Data')
